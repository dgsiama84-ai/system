...
select 
  coalesce(
    (select sum(total) from transactions where type='sale' and date(created_at)=current_date),0
  )
  -
  coalesce(
    (select sum(amount) from expenses where date(created_at)=current_date),0
  ) as profit;
